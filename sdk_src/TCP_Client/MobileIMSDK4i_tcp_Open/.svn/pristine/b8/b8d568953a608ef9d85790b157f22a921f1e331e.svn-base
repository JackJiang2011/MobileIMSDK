...
@implementation MBGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  ...
  MBGCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			MBGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
{
	MBGCDAsyncSocket *asyncSocket = (__bridge MBGCDAsyncSocket *)connection;
...
+ (void)scheduleCFStreams:(MBGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(MBGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	MBGCDAsyncSocket *asyncSocket = (__bridge MBGCDAsyncSocket *)pInfo;
