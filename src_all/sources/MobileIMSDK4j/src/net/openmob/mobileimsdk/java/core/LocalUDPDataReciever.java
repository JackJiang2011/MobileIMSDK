...
public class LocalUDPDataReciever
{
  ...
  private static class MessageHandler
  {
    ...
    private void sendRecievedBack(Protocal pFromServer)
    {
      if (pFromServer.getFp() != null)
      {
        new LocalUDPDataSender.SendCommonDataAsync(
          ...
          pFromServer.getTo(), 
          pFromServer.getFrom(), 
          pFromServer.getFp()), pFromServer)
        {
          protected void onPostExecute(Integer code)
          {
            if (ClientCoreSDK.DEBUG)
              Log.d(LocalUDPDataReciever.TAG, "【IMCORE】【QoS】向" + this.val$pFromServer.getFrom() + "发送" + this.val$pFromServer.getFp() + "包的应答包成功,from=" + this.val$pFromServer.getTo() + "！"); 
          }
        }
        .execute();
      }
