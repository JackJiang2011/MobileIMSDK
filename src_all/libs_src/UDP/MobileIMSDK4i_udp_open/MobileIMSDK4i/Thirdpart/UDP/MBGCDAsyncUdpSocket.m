...
@implementation MBGCDAsyncUdpSocket
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	GCDAsyncUdpSendPacket *packet = [[GCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [MBGCDAsyncUdpSocket familyFromAddress:remoteAddr];
...
+ (void)addStreamListener:(MBGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(MBGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		MBGCDAsyncUdpSocket *asyncUdpSocket = (__bridge MBGCDAsyncUdpSocket *)pInfo;
