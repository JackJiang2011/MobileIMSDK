 ...
 * 协议报文对象.
 * <p>
 * <b>重要说明：</b>因本类中的属性retryCount仅用于本地（不需要通过网络把值传给接收方），因为在对象转JSON时应去掉此属性，那么接收方收到本对象并反序列化时该属性正好就是默认值0.
 *
 ...
 * 补充：理论上应用层不参与本字段的定义，可将其视为透明，如需定义应用层的消息类型，请使用 {@link typeu} 字段并配合dataContent一起使用。
 * 默认：0
 *
...
/*!
 * 意义：协议数据内容。
 * 说明：本字段用于MobileIMSDK_X框架中时，可能会存放一些指令内容。当本字段用于应用层时，由用户自行定义和使用其内容 */
 ...
 * 默认：false
 *
 * @warning 当本属性申明为BOOL类型时，在模拟器iPhone 4s、iPhone 5时，利用官方的NSJSONSerialization类转成JSON时，会被解析成0或1，
 * 而在iPhone 5s和iPhone 6上会被解析成true或false，符合JSON规范的应是true和false. 把申明改成bool型时，在4s、5、5s、6上都能正常解析成true和false！
 */
 ...
 * 它存在重复的可能性有2种：
 * <ul>
 * 	<li>1) 比如在客户端生成时如果生成过快的话（时间戳最小单位是1毫秒，如1毫秒内生成多个指纹码），理论上是有重复可能性；</li>
